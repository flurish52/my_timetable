...
// Each record: { id, programme_id, day_of_week, start_time, end_time, programme: { name, venue, ... } }
const timetableMap = computed(() => {
    ...
    for (const day of DAYS) {
        ...
        // Filter records for this day (DB stores e.g. 'Monday', normalise to lowercase)
        ...
        for (const hourSlot of HOUR_SLOTS) {
            ...
            // Find a record whose time range overlaps this hour slot
            ...
            // Normalise into the same shape the template already expects
    ...
    <GuestLayout>
        <div class="flex px-4 md:px-12 flex-col h-full bg-white">
            ...
            <div v-if="!timetable.length" class="flex-1 flex items-center justify-center">
                <p class="text-gray-400 text-sm font-medium">No timetable data available.</p>
            </div>
                ...
                <table class="border-collapse" style="min-width: 640px; width: 100%;">
                    ...
                    >
                        ...
                        <td
                            v-for="day in DAYS"
                            :key="day"
                            class="border-b border-r border-gray-100 last:border-r-0 px-3 h-16 align-middle"
                        ...
                        >
                            <template v-if="timetableMap[day]?.[hourSlot.start]">
                                <div :class="[
                                    ...
                                    <!-- Course code -->
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wide leading-none">
                                        {{ timetableMap[day][hourSlot.start].code }}
                                    </p>

                                    <!-- Course title -->
                                    <p :class="[
                                        'text-[12px] font-bold leading-tight mt-0.5',
