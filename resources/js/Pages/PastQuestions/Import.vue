...
const METHODS = [
    {
        key: 'scan',
        label: 'Scan from an image',
        description: 'Faster if you already have the image at hand or a photo of the past question, snap and upload',
...
function handleExcelFile(e) {
    ...
    router.post(route('past-questions.import-excel', props.pastQuestion.id), {file}, {
        forceFormData: true,
        onError: (errs) => {
            uploadError.value = Object.values(errs)[0] ?? 'Import failed.'
        },
        onFinish: () => {
    ...
    <div class="min-h-screen bg-neutral-50">
        ...
        <div class="max-w-3xl mx-auto px-4 py-8">
            ...
            <div v-if="activeMethod !== 'paste'">
                ...
                <p v-if="uploadError"
                   class="mt-4 mt-1 text-sm text-red-600 bg-red-50 border border-red-200 rounded-md px-3 py-2">
                    {{ uploadError }}
                </p>
                ...
                <div class="grid sm:grid-cols-2 gap-3">
                    ...
                    >
                        <div
                            ...
                            <div
                                :class="`w-9 h-9 rounded-lg flex items-center justify-center mb-3 ${m.iconClasses}`">
                                <svg v-if="m.key === 'paste'" class="w-4.5 h-4.5" viewBox="0 0 20 20"
                                     fill="currentColor">
                                    <path fill-rule="evenodd"
                                          d="M6 3a1 1 0 011-1h6a1 1 0 011 1v1h1a2 2 0 012 2v11a2 2 0 01-2 2H5a2 2 0 01-2-2V6a2 2 0 012-2h1V3zm2 0v1h4V3H8zM5 6v11h10V6H5zm2 3a1 1 0 100 2h6a1 1 0 100-2H7zm0 4a1 1 0 100 2h4a1 1 0 100-2H7z"
                                          clip-rule="evenodd"/>
                                </svg>
                                <svg v-else-if="m.key === 'excel'" class="w-4.5 h-4.5" viewBox="0 0 20 20"
                                     fill="currentColor">
                                    <path fill-rule="evenodd"
                                          d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm7 9a1 1 0 10-2 0v.01a1 1 0 102 0V13zm-4-2a1 1 0 011 1v.01a1 1 0 11-2 0V12a1 1 0 011-1zm4-2a1 1 0 10-2 0v.01a1 1 0 102 0V9zm-4-2a1 1 0 011 1v.01a1 1 0 11-2 0V8a1 1 0 011-1z"
                                          clip-rule="evenodd"/>
                                </svg>
                                <svg v-else class="w-4.5 h-4.5" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd"
                                          d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm8 1.414L14.586 8H12V5.414zM6 10a1 1 0 100 2h8a1 1 0 100-2H6zm0 4a1 1 0 100 2h5a1 1 0 100-2H6z"
                                          clip-rule="evenodd"/>
                                </svg>
                            </div>
                        ...
                        <div v-else>
                            <ScanCTA
                                ...
                                buttonText="Import questions from an image"
                ...
                <input ref="excelFileInput" type="file" accept=".xlsx,.xls,.csv" class="hidden"
                       @change="handleExcelFile"/>
                <input ref="wordFileInput" type="file" accept=".doc,.docx" class="hidden"
                       @change="(e) => handleDocumentFile('word', e)"/>
                <input ref="pdfFileInput" type="file" accept=".pdf" class="hidden"
                       @change="(e) => handleDocumentFile('pdf', e)"/>
